...
export default function AdminBookingsPage() {
  ...
  const pageSize = 10; // Puedes cambiar el tamaño aquí o volverlo dinámico

  // Volver a disparar la petición cada vez que cambie la página actual
  useEffect(() => {
    fetchBookings(currentPage);
  }, [currentPage]);

  const fetchBookings = async (pageToFetch = 0) => {
    try {
      setLoading(true);
      setError(null);
      // Enviamos la página solicitada al backend
      const data = await getAllBookings(pageToFetch, pageSize);
      
      setBookings(data.content); // Guardamos la lista de elementos
      setTotalPages(data.totalPages); // Guardamos páginas totales
      setTotalElements(data.totalElements); // Guardamos elementos totales
    } catch (err) {
  ...
  // El cálculo de estadísticas ahora se aplica sobre los elementos de la página actual.
  // (Nota: Si necesitas estadísticas globales reales, tu backend debería proveer un endpoint de /stats)
  ...
  // Se mantienen tus filtros locales sobre el "content" de la página actual
  ...
  const exportToCSV = () => {
    const headers = ['ID', 'Cliente', 'Email', 'Apartamento', 'Fecha Inicio', 'Fecha Fin', 'Huéspedes', 'Precio Total', 'Estado', 'Notas'];
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent><div className="space-y-4">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div></CardContent>
        </Card>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <Button onClick={() => fetchBookings(currentPage)} className="mt-4">
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Reservas (Pág)</CardTitle>
              <Calendar className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{stats.total}</div>
              <p className="text-xs text-muted-foreground">{stats.upcomingCheckIns} check-ins próx.</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Ingresos (Pág)</CardTitle>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Reservas en base de datos</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium">Buscar en esta página</label>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Mostrando {filteredAndSortedBookings.length} de {totalElements} totales
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
                      ...
                      <TableRow key={booking.id} className="hover:bg-muted/50">
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            ...
                            {booking.client?.email && <div className="text-sm text-muted-foreground">{booking.client.email}</div>}
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            <div className="text-sm">{format(parseISO(booking.startDate), 'dd MMM yyyy', { locale: es })}</div>
                            <div className="text-sm text-muted-foreground">{format(parseISO(booking.endDate), 'dd MMM yyyy', { locale: es })}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <Users className="h-4 w-4 text-muted-foreground" /> {booking.guests}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1 font-medium">
                            <Euro className="h-4 w-4 text-muted-foreground" /> {booking.totalPrice.toFixed(2)}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onClick={() => alert(`Ver detalles de reserva ID: ${booking.id}`)}><Eye className="h-4 w-4 mr-2" /> Ver detalles</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => alert(`Editar reserva ID: ${booking.id}`)}><Edit className="h-4 w-4 mr-2" /> Editar</DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => handleDeleteClick(booking)} className="text-destructive"><Trash2 className="h-4 w-4 mr-2" /> Eliminar</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </TableCell>
                      </TableRow>
            ...
            {/* BARRA DE NAVEGACIÓN DE PAGINACIÓN (NUEVA) */}
            {totalPages > 1 && (
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {bookingToDelete?.client && <span className="block mt-2 font-medium">Cliente: {bookingToDelete.client.name}</span>}
              Esta acción no se puede deshacer.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteDialogOpen(false)}>Cancelar</Button>
            <Button variant="destructive" onClick={handleDeleteConfirm}><Trash2 className="h-4 w-4 mr-2" /> Eliminar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
