...
import { getAllApartmentIds, getDiscountsByApartmentId, type Discount } from "@/api/discount"
...
export default function AdminDiscountsPage() {
  ...
  const daysInfo: DayInfo[] = useMemo(() => {
    return calendarDays.map((date) => {
      ...
      const maxDiscount = hasDiscount ? Math.max(...dayDiscounts.map((d) => (d.isPercentage ? d.discount : 0))) : 0
  ...
  const stats = useMemo(() => {
    ...
    const avgDiscount =
      discounts.length > 0
        ? discounts.filter((d) => d.isPercentage).reduce((sum, d) => sum + d.discount, 0) /
  ...
  const getDayClassName = (dayInfo: DayInfo) => {
    ...
    if (dayInfo.hasDiscount) {
      if (dayInfo.maxDiscount >= 30) {
        ...
        statusClasses = "bg-yellow-100 border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-800"
      } else {
  ...
  const getDiscountBadge = (discount: Discount) => {
    return (
      <Badge variant={discount.isPercentage ? "default" : "secondary"} className="text-xs">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Gestiona los descuentos de los apartamentos</p>
          </div>
        ...
        {selectedApartmentId ? (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">Días con Descuento</CardTitle>
                  <Info className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{stats.daysWithDiscount}</div>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">Descuentos porcentuales</p>
                </CardContent>
              </Card>
            </div>
                          ...
                          <TooltipTrigger asChild>
                            <div className={getDayClassName(dayInfo)}>
                              <div className="flex justify-between items-start">
                                ...
                                {dayInfo.hasDiscount && <Tag className="h-3 w-3 text-green-600" />}
                              </div>
                              {dayInfo.isToday && (
                              ...
                              {dayInfo.discounts.length > 0 && (
                                <div className="mt-1 space-y-1">
                                  {dayInfo.discounts.slice(0, 2).map((discount, i) => (
                                    <div key={i} className="text-xs p-1 bg-background/80 rounded truncate">
                                      {discount.isPercentage ? `${discount.discount}%` : `${discount.discount}€`}
                                    </div>
                                  ))}
                            ...
                            <div className="space-y-2">
                              ...
                              {dayInfo.discounts.length > 0 ? (
                                <div className="space-y-1">
                                  <div className="text-sm font-medium">Descuentos activos:</div>
                                  {dayInfo.discounts.map((discount, i) => (
                                    <div key={i} className="text-sm flex items-center gap-2">
                                      • {getDiscountBadge(discount)}
                                      <span className="text-xs text-muted-foreground">
                                        ({format(parseISO(discount.startDate), "dd/MM")} -{" "}
                                        {format(parseISO(discount.endDate), "dd/MM")})
                                      </span>
                                    </div>
                                  ))}
                                </div>
                              ) : (
                                <div className="text-sm text-muted-foreground">Sin descuentos</div>
                              )}
                            </div>
                          </TooltipContent>
                        </Tooltip>
              ...
              <TabsContent value="list">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Descuentos para el apartamento #{selectedApartmentId}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {discounts.length === 0 ? (
                      <div className="text-center py-12">
                        <Tag className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                        <h3 className="text-lg font-semibold mb-2">No hay descuentos</h3>
                        <p className="text-muted-foreground">Este apartamento no tiene descuentos configurados.</p>
                      </div>
                    ) : (
                          ...
                          <div key={discount.id} className="flex items-center justify-between p-4 border rounded-lg">
                            <div className="space-y-1">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {format(parseISO(discount.startDate), "dd MMM yyyy", { locale: es })} -{" "}
                                {format(parseISO(discount.endDate), "dd MMM yyyy", { locale: es })}
                              </div>
                            </div>
                            <div className="text-right">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {discount.isPercentage ? "Porcentaje" : "Cantidad fija"}
                              </div>
                            </div>
                          </div>
                        ))}
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted-foreground text-center">
                Elige un apartamento del selector de arriba para ver su calendario de descuentos.
              </p>
            </CardContent>
          </Card>
        )}
