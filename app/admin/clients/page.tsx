...
export default function AdminPricesPage() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const data = await getPricesByMonth(selectedApartment, currentMonth, currentYear)
      console.log("Refreshed prices:", data)
      setPrices(data)
      setError(null)
    } catch (err) {
  ...
  const handleSavePrices = async () => {
    ...
    try {
      ...
      if (Number.parseInt(apartmentIdForDays) === selectedApartment) {
        await fetchPrices()
      }
    ...
    <div className="flex min-h-screen flex-col">
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <PricesCalendar
          ...
          onBulkUpdatePrices={function (dates: string[], price: number): Promise<void> {
